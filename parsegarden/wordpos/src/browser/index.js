...
class WordPOS {
    ...
    initFiles() {
        ...
        const loadOne = (Comp, pos) =>
            new Comp(this.options.dictPath, POS[pos], this.options)
        const loader = Comp => keys.map(loadOne.bind(null, Comp))
        const reducer = arr =>
            arr.reduce((coll, item, i) => ((coll[keys[i]] = item), coll), {})
    ...
    preloadFiles(pos) {
        ...
        if (this.options.includeData) {
            promise = Promise.all(
                [].concat(promise, this._preload(this.dataFiles, pos)),
            ).then(res => flat(res))
        }
        return promise
    }
    ...
    _preload(files, pos) {
        ...
        return (
            promise ||
            Promise.reject(new RangeError(`Unknown POS "${pos}" for preload.`))
        )
    }
