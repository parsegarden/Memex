...
function lookupPOS(word, callback) {
    ...
    return Promise.all(methods.map(exec))
        .then(done)
        .catch(error)
...
function seek(offset, pos, callback) {
    ...
    if (isNaN(offsetTmp) || offsetTmp <= 0)
        return error(
            'Offset must be valid positive number: ' + offset,
            callback,
        )
    ...
    if (!data)
        return error(
            'Incorrect POS - 2nd argument must be a, r, n or v.',
            callback,
        )
